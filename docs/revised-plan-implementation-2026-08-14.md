...
| Widget | Small and medium Home Screen layouts plus Inline, Circular, and Rectangular Lock Screen layouts use the current daily set. Each timeline refresh advances one word in order and wraps around; tapping a widget opens Today’s words directly, while Medium shows the complete Definition or Example selected in Settings. |
