...
- Small and medium Home Screen widgets, plus inline, circular, and rectangular Lock Screen widgets, use the current daily set. Widget timelines advance through it in order and wrap around to the first word; tapping a widget opens Today’s words directly and does not start Quiz; no network lookup is required.
