...
- Small and medium Home Screen plus Inline, Circular, and Rectangular Lock Screen
  WidgetKit study widgets backed by an App Group snapshot. The small widget is
  word-first; tapping any widget opens Today’s words rather than Quiz; the
  medium widget places the complete selected definition or example below the
  word, with Definition as the default setting.
